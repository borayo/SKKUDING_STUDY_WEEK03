import { pokemonData, Pokemon } from './pokemon.js';

const raw = localStorage.getItem("selectedPokemon");

if (raw) {
  const pokemon = JSON.parse(raw) as Pokemon;
  const index = pokemonData.findIndex((p) => p.name === pokemon.name);

  const image = document.getElementById("pokemon-image") as HTMLImageElement | null;
